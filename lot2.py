...
async def save_to_sheet_admin(user_id, context):
    ...
    try:
        ...
        # ── Fan out services & locations into relational tables ──────────
        try:
            ...
            if new_id:
                ...
                service_rows = [
                    {"business_id": new_id, "service_category": s.strip()}
                    for s in raw_service.split(',') if s.strip()
                ]
                if service_rows:
                    await asyncio.to_thread(
                ...
                logging.info(f"✅ Linked {len(service_rows)} service(s) and {len(location_rows)} location(s) for business {new_id}")
