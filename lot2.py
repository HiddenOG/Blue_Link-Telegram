...
def h_esc(text):
    """Alias for html.escape to keep code clean"""
    if not text: return ""
    return html.escape(str(text))

def safe_html(text):
    """Robust conversion of Gemini Markdown to Telegram HTML"""
    if not text: return ""
    # 1. Escape basic HTML entities
    res = h_esc(text)
    # 2. Convert Bold (Pairs of **)
    parts = res.split('**')
    final = []
    for i, part in enumerate(parts):
        final.append(f"<b>{part}</b>" if i % 2 == 1 else part)
    res = "".join(final)
    # 3. Convert Italics (Pairs of __)
    parts = res.split('__')
    final = []
    for i, part in enumerate(parts):
        final.append(f"<i>{part}</i>" if i % 2 == 1 else part)
    res = "".join(final)
    # 4. Convert single * to italics if balanced
    if res.count('*') % 2 == 0:
        parts = res.split('*')
        final = []
        for i, part in enumerate(parts):
            final.append(f"<i>{part}</i>" if i % 2 == 1 else part)
        res = "".join(final)
    # 5. Clean up any empty tags
    return res.replace('<b></b>', '').replace('<i></i>', '')
...
"""
    try:
        ...
        # If Gemini is too conversational, find all numbers in the text
        if result_text.lower() == "none" or not any(c.isdigit() for c in result_text):
            results = []
        else:
            # Extract only digits and ignore explanations
            raw_indices = re.findall(r'\d+', result_text)
            indices = []
            for s_idx in raw_indices:
                try:
                    idx = int(s_idx)
                    if 0 <= idx < len(business_data):
                        indices.append(idx)
            ...
            results = [business_data[i] for i in indices]
...
def format_matches_reply(matches, customer_id=None, is_group=False):
    """Format search results with Request Service buttons (Mini App for Private, URL for Groups)"""
    if not matches:
        return None, None
    ...
    reply = "✅ <b>Here are some businesses that match your request:</b>\n\n"
    buttons = []
    
    for row, _ in sorted_matches:
        business_id = row.get('id')
        business_name = h_esc(get_row_value(row, 'Business Name') or 'Unknown')
        location = h_esc(get_row_value(row, 'Business Location') or 'N/A')
        services = h_esc(get_row_value(row, 'Business Services') or 'N/A')
        
        reply += f"🏷️ <b>{business_name}</b>\n"
        reply += f"📍 {location}\n"
        reply += f"🛠️ {services}\n"
        if is_ad_boosted(row):
            reply += f" ⭐ <b>FEATURED AD</b>\n"
        
        telegram_id = row.get('telegram_id')
        if not telegram_id:
            reply += f"📜 <i>Legacy Listing</i>\n"
        ...
        if phone:
            ...
            elif not phone.startswith('234'): phone = '234' + phone
        ...
        if business_id and phone:
            base_url = WEB_APP_URL if WEB_APP_URL.startswith('http') else 'https://' + WEB_APP_URL
            profile_url = f"{base_url}/business.html?id={business_id}"
            
            if customer_id:
                profile_url += f"&uid={customer_id}"
            if "ngrok-free.app" in profile_url:
                profile_url += "&ngrok-skip-browser-warning=true"
            
            button_text = f"📞 Request Service - {business_name[:20]}" if row.get('telegram_id') else f"💬 Contact - {business_name[:20]}"
            
            if is_group:
                # Use regular URL button for groups (Mini Apps not allowed)
                buttons.append([
                    InlineKeyboardButton(button_text, url=profile_url)
                ])
            else:
                # Use Mini App button for private chats
                buttons.append([
                    InlineKeyboardButton(button_text, web_app=WebAppInfo(url=profile_url))
                ])
...
async def search_and_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply, keyboard = format_matches_reply(matches, customer_id=user_id, is_group=False)
...
async def inline_search_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Clean inline search (no redirect button when empty)"""
    query = update.inline_query.query.strip()

    if not query:
        # Avoid showing "Switch to PM" buttons as they confuse users in groups
        await update.inline_query.answer([], cache_time=1)
        return

    # QUICK FALLBACK: Always offer "Chat" result immediately to avoid "missing send button"
    results = [
        InlineQueryResultArticle(
            id=f"chat_{uuid.uuid4()}",
            title=f"Chat: \"{query}\"",
            description="Send this as a message to the bot",
            input_message_content=InputTextMessageContent(f"@{BOT_USERNAME} {query}")
        )
    ]

    # For directory search, only run if the query is long enough to be a real search
    # This reduces heavy Gemini calls during every keystroke
    if len(query) > 3:
        try:
            # Short timeout/top_k to avoid "Query is too old" errors
            matches = smart_directory_search(query, top_k=3)
            for row, _ in matches:
                ...
                results.append(
                    InlineQueryResultArticle(
                        ...
                        description=f"{service} • tap to share",
...
async def fallback_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        reply, keyboard = format_matches_reply(matches, customer_id=update.effective_user.id, is_group=(update.effective_chat.type != 'private'))
    ...
    # ✅ FIX: Avoid recommending /find or buttons in group replies
    if reply and reply.strip():
        if update.effective_chat.type == "private":
            markup = get_main_keyboard(update.effective_user.id)
            await update.message.reply_text(reply, reply_markup=markup)
        else:
            # Strictly No Buttons in Groups
            reply = reply.replace('Tap "🔍 Find a Service"', 'Just ask me to find a service')
            reply = reply.replace('type /find', 'just ask me directly')
...
# Bot username: @Blue_serviceBot
# ============================================================

# Keywords that suggest a user is asking for a service
GROUP_SERVICE_KEYWORDS = re.compile(
    r'\b(i need|looking for|find me|find a|get me|where can i (?:get|find)|'
    r'anyone know(?: a)?|recommend(?: a)?|need a|need an|searching for|'
    r'who (?:does|can do|offers)|any (?:good )?)\b',
    re.IGNORECASE
)

BOT_USERNAME = "Blue_serviceBot"  # without @


async def group_welcome_new_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Welcome new members when they join a group where the bot is admin."""
    ...
    for member in update.message.new_chat_members:
        ...
        if member.id == context.bot.id:
            # Bot was just added — introduce itself
            await update.message.reply_text(
                "👋 Hi everyone! I'm <b>BlueLink</b> — your group's business directory assistant.\n\n"
                "I can help you:\n"
                "🔍 Find local services (just say \"I need a plumber in Lagos\")\n"
                "📋 Answer questions about businesses on BlueLink\n"
                "💬 General questions — just @mention me!\n\n"
                "Tag me anytime: @Blue_serviceBot",
                parse_mode='HTML'
            )
            continue

        first_name = h_esc(member.first_name or "there")
        await update.message.reply_text(
            f"👋 Welcome, <b>{first_name}</b>!\n\n"
            f"I'm <b>BlueLink</b> — the group's business directory bot.\n"
            f"Need a service provider? Just tell me what you're looking for!\n"
            f"Example: <i>\"I need a plumber in Lagos\"</i>\n\n"
            f"Or tag me directly: @Blue_serviceBot",
            parse_mode='HTML'
        )


async def group_find_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /find in groups — gives instructions + link to private chat."""
    await update.message.reply_text(
        "🔍 <b>How to find services:</b>\n\n"
        "1️⃣ <b>In this group</b>: Just ask naturally!\n"
        "Example: <i>\"I need a plumber in Lagos\"</i>\n\n"
        "2️⃣ <b>Private Menu</b>: <a href=\"https://t.me/Blue_serviceBot?start=find\">Message me privately</a> "
        "if you prefer using my search buttons! 📱",
        parse_mode='HTML',
        disable_web_page_preview=True
    )

async def group_register_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /register in groups — redirects to private chat."""
    await update.message.reply_text(
        "🏪 <b>Business Registration:</b>\n\n"
        "To register your business, please message me privately at @Blue_serviceBot.\n"
        "This keeps the group chat clean and protects your details! 🔒",
        parse_mode='HTML'
    )

async def group_mention_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Respond when the bot is @mentioned or when a user replies to the bot's message.
    """
    ...
    # Strip the bot mention so we search cleaner text
    query = re.sub(r'@Blue_serviceBot\b', '', raw_text, flags=re.IGNORECASE).strip()
    if not query:
        await update.message.reply_text(
            "Hey! 👋 I'm here. Just ask me to find a service or ask any question.",
            parse_mode='HTML'
        )
        return

    # Check if it looks like a service request
    if GROUP_SERVICE_KEYWORDS.search(query):
        await _group_service_search(update, context, query, user_id)
    else:
        ...
        # Avoid recommending /find or buttons in group replies
        if reply:
            reply = reply.replace('Tap "🔍 Find a Service"', 'Just ask me to find a service')
            reply = reply.replace('type /find', 'just ask me directly')
            reply = safe_html(reply)
...
async def group_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Listen to ALL group messages (no mention needed).
    Only acts when the message matches service-seeking keywords.
    Ignores messages that explicitly @mention the bot (handled by group_mention_handler).
    """
    if not update.message or not update.message.text:
        return
    ...
    # Skip if this already contains a bot mention — that handler fires first
    if BOT_USERNAME.lower() in text.lower():
        return

    # Only react to service-seeking patterns
    if not GROUP_SERVICE_KEYWORDS.search(text):
        return

    user_id = update.effective_user.id
    await _group_service_search(update, context, text, user_id)


async def _group_service_search(update: Update, context: ContextTypes.DEFAULT_TYPE,
                                 query: str, user_id: int):
    """
    Run smart_directory_search for a group message and reply with results.
    Falls back to Gemini if no matches found.
    """
    await update.message.reply_chat_action("typing")
    ...
    try:
        ...
        logging.error(f"Group service search error: {e}")
        matches = []

    if matches:
        reply_text, keyboard = format_matches_reply(matches, customer_id=user_id, is_group=True)
        if reply_text:
            await update.message.reply_text(
                reply_text,
                parse_mode='HTML',
    ...
    # No matches — use Gemini to give a helpful reply
    gemini_reply = await ask_gemini(
        f"[Group chat - service request not found in directory] {query}",
        user_id=user_id
    )
    if gemini_reply:
        gemini_reply = gemini_reply.replace('Tap "🔍 Find a Service"', 'Just ask me to find a service')
        gemini_reply = gemini_reply.replace('type /find', 'just ask me directly')
        gemini_reply = safe_html(gemini_reply)
    
    await update.message.reply_text(gemini_reply, parse_mode='HTML')
...
def main():
    ...
    # Redirect administrative commands in groups to keep the chat clean
    application.add_handler(CommandHandler(['register', 'dashboard'], group_register_command, filters=filters.ChatType.GROUPS))
    ...
    # Redirect /find in groups to instructions (disabling menu flow)
    application.add_handler(CommandHandler('find', group_find_command, filters=filters.ChatType.GROUPS))
    ...
    # ---------------------------------------------------------------
    # GROUP CHAT ADMIN HANDLERS (@Blue_serviceBot)
    # Must be added BEFORE the private fallback_chat handler
    # ---------------------------------------------------------------

    # 1. Welcome new members (fires on StatusUpdate.NEW_CHAT_MEMBERS)
    application.add_handler(MessageHandler(
        filters.ChatType.GROUPS & filters.StatusUpdate.NEW_CHAT_MEMBERS,
        group_welcome_new_members
    ))

    # 2. Respond when @mentioned OR when user replies to the bot's message
    application.add_handler(MessageHandler(
        filters.ChatType.GROUPS & filters.TEXT & ~filters.COMMAND & (
            filters.Entity("mention") | 
            filters.REPLY | 
            filters.Regex(re.compile(f'@{BOT_USERNAME}', re.IGNORECASE))
        ),
        group_mention_handler
    ))

    # 3. Detect service requests from ANY group message (no mention needed)
    application.add_handler(MessageHandler(
        filters.ChatType.GROUPS & filters.TEXT & ~filters.COMMAND,
