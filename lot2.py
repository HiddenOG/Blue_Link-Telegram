...
if "ngrok-free.app" in WEB_APP_URL and "ngrok-skip-browser-warning" not in WEB_APP_URL:
    ...
    DASHBOARD_URL = f"{WEB_APP_URL}{sep}ngrok-skip-browser-warning=true"
    # For register.html, we must append the path BEFORE the query params
    REGISTER_URL = f"{WEB_APP_URL}/register.html{sep}ngrok-skip-browser-warning=true"
    CATALOG_URL = f"{WEB_APP_URL}/catalog.html{sep}ngrok-skip-browser-warning=true"
else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    # Clear conversation memory on /start for fresh context
...
async def find_service_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if rows:
        logging.info(f"Sample row keys: {list(rows[0].keys())}")
    
    # ✅ FIX: Use lowercase snake_case column names from Supabase
    services = set()
    for row in rows:
        # Supabase returns lowercase snake_case: 'business_services'
        service = str(row.get('business_services', '')).strip()
        if service:
            services.add(service)
    
    services = sorted(services)
    
    # ✅ DEBUG: Log extracted services
...
async def ask_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ FIX: Use lowercase snake_case column names
    rows = get_cached_businesses()
    ...
    locations = set()
    for row in rows:
        # Supabase returns: 'business_services' and 'business_location'
...
async def save_to_sheet_admin(user_id, context):
    ...
    try:
        ...
        # ✅ ✅ ✅ NEW: Give 5 Blue Coins on FIRST business approval
