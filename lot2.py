...
async def inline_search_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query:
        # In groups, showing a "Switch to PM" button is confusing when users just want to tag the bot.
        # Removing the button makes the inline UI less intrusive.
    ...
    # ✅ Add a "Chat" result so users can always send their query even if no businesses match.
    # This solves the "missing send button" issue in inline mode.
    results.append(
...
async def group_mention_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Runs a service search if the message looks like a service request,
    ...
    if not query:
        await update.message.reply_text(
            "Hey! 👋 How can I help? Ask me to find a service or ask any question.",
            parse_mode='Markdown'
        )
        return
