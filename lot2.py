...
async def group_mention_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if GROUP_SERVICE_KEYWORDS.search(query):
        ...
        if reply:
            reply = reply.replace('Tap "🔍 Find a Service"', 'Just ask me to find a service')
            reply = reply.replace('type /find', 'just ask me directly')
            # Use standardized HTML conversion
            reply = h_esc(reply)
            reply = reply.replace('**', '<b>').replace('</b>*', '</b>').replace('*<b>', '<b>')
            reply = reply.replace('**', '<b>').replace('__', '<i>')
            reply = reply.replace('&lt;b&gt;', '<b>').replace('&lt;/b&gt;', '</b>')
            reply = reply.replace('&lt;i&gt;', '<i>').replace('&lt;/i&gt;', '</i>')
...
async def _group_service_search(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    if matches:
        reply_text, keyboard = format_matches_reply(matches, customer_id=user_id)
        if reply_text:
            await update.message.reply_text(
                reply_text,
                parse_mode='HTML',
    ...
    if gemini_reply:
        ...
        # Standardized HTML conversion logic
        gemini_reply = h_esc(gemini_reply)
        gemini_reply = gemini_reply.replace('**', '<b>').replace('</b>*', '</b>').replace('*<b>', '<b>')
        gemini_reply = gemini_reply.replace('**', '<b>').replace('__', '<i>')
        gemini_reply = gemini_reply.replace('&lt;b&gt;', '<b>').replace('&lt;/b&gt;', '</b>')
        gemini_reply = gemini_reply.replace('&lt;i&gt;', '<i>').replace('&lt;/i&gt;', '</i>')
