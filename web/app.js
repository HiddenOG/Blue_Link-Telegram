...
let photoState = {
    ...
    objectUrls: new Map() // File -> url mapping for memory management
};
...
function showEditPage() {
    ...
    // Load photos
    photoState = { existing: [], removed: [], newFiles: [], objectUrls: new Map() };
...
function renderPhotoManager() {
    ...
    photoState.newFiles.forEach((file, idx) => {
        ...
        let url = photoState.objectUrls.get(file);
        if (!url) {
            url = URL.createObjectURL(file);
            photoState.objectUrls.set(file, url);
        }
...
function hideEditPage() {
    // Cleanup object URLs on exit
    photoState.objectUrls.forEach(url => URL.revokeObjectURL(url));
    photoState.objectUrls.clear();
...
document.getElementById('photo-manager').addEventListener('click', (e) => {
    ...
    if (type === 'existing') {
        ...
        const removedFile = photoState.newFiles.splice(index, 1)[0];
        // Cleanup object URL
        const url = photoState.objectUrls.get(removedFile);
        if (url) {
            URL.revokeObjectURL(url);
            photoState.objectUrls.delete(removedFile);
        }
    }
    renderPhotoManager();
});
...
// Reset input value on click to ensure change event fires every time
document.getElementById('photo-upload').onclick = function () {
    this.value = null;
};

document.getElementById('photo-upload').addEventListener('change', (e) => {
    const files = Array.from(e.target.files);
    alert(`Files selected for edit: ${files.length}`);
    const totalPhotos = photoState.existing.length + photoState.newFiles.length;
