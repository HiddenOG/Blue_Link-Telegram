...
async def get_dashboard_data(initData: str = Query(None), user_id: int = Query(None)):
    ...
    for brief in businesses_brief:
        ...
        # Fetch services and locations from relational tables
        services_list = []
        locations_list = []
        try:
            ...
            services_list = [r['service_category'] for r in (svc_resp.data or [])]
        except Exception:
            pass
        ...
        try:
            ...
            locations_list = [r['location'] for r in (loc_resp.data or [])]
        except Exception:
            pass

        # Fallback: if relational tables empty, split existing flat field
        if not services_list and brief.get('service'):
            services_list = [s.strip() for s in brief['service'].split(',') if s.strip()]
        if not locations_list and brief.get('location'):
            locations_list = [l.strip() for l in brief['location'].split(',') if l.strip()]
...
async def get_business_profile(business_id: int):
    ...
    # Fetch approved reviews for star rating
    avg_rating = None
    review_count = 0
    try:
        reviews_resp = await asyncio.to_thread(
        ...
        if reviews_resp.data:
            review_count = len(reviews_resp.data)
            total_stars = sum(r['stars'] for r in reviews_resp.data)
            avg_rating = round(total_stars / review_count, 1)
    except Exception as e:
        logging.warning(f"Could not fetch reviews for business {business_id}: {e}")
    ...
    return {
        ...
        "is_reported": bool(get_row_value(business, 'is_reported')),
...
async def report_business(request: Request):
    """Handle business report submission and notify admin."""
    from bot_state import get_cached_businesses, get_row_value
    ...
    if not business_id or not reason:
        raise HTTPException(status_code=400, detail="Missing business_id or reason")
        
    # Find business details for the report
    cached = get_cached_businesses()
    business = None
    for b in cached:
        if str(get_row_value(b, 'id')) == str(business_id):
    ...
    biz_name = get_row_value(business, 'Business Name') or 'Unknown'
    biz_desc = get_row_value(business, 'Business Description') or 'No description'
    biz_phone = get_row_value(business, 'Phone Number') or 'N/A'
    biz_loc = get_row_value(business, 'Business Location') or 'N/A'
    ...
    try:
        ...
        await asyncio.to_thread(
            lambda: supabase.table('scam_reports').insert(report_data).execute()
        )
        
        # 2. Notify Admin via bot
        admin_id = os.getenv("ADMIN_ID")
        if bot_instance and admin_id:
            msg = (
                f"🚨 *NEW BUSINESS REPORT*\n\n"
                f"🏢 *Business:* {biz_name}\n"
                f"🆔 *ID:* `{business_id}`\n"
                f"📍 *Location:* {biz_loc}\n"
                f"📞 *Phone:* `{biz_phone}`\n"
                f"📝 *Description:* {biz_desc[:150]}...\n\n"
                f"🚩 *REASON FOR REPORT:*\n{reason}\n\n"
                f"👤 *Reporter:* {reporter_id or 'Anonymous'}\n"
            )
            
            # Add "Mark as Seen (Flag)" button
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Mark as Seen (Flag Reported)", callback_data=f"flag_scam_{business_id}")]
            ])
            ...
            try:
                ...
                logging.error(f"Failed to notify admin of report: {e}")
...
async def get_catalog(service: Optional[str] = Query(None), location: Optional[str] = Query(None)):
    ...
    - No params: returns list of unique services (from businesses table + business_services table)
    - ?service=X: returns unique locations for that service (from both tables)
    - ?service=X&location=Y: returns businesses in that service+location (from both tables)
    """
    from bot_state import get_cached_businesses, get_row_value

    if service and location:
        # Return businesses matching service + location
        # Check main table AND relational tables
        matched_ids = set()
        results = []

        # Step 1: Businesses where main service+location matches
        businesses = get_cached_businesses()
        for biz in businesses:
            biz_service = (get_row_value(biz, 'Business Services') or '').strip().lower()
            biz_location = (get_row_value(biz, 'Business Location') or '').strip().lower()
            if biz_service == service.strip().lower() and biz_location == location.strip().lower():
                biz_id = get_row_value(biz, 'id')
                if biz_id not in matched_ids:
                    matched_ids.add(biz_id)
                    results.append({
                        "id": biz_id,
                        "business_name": get_row_value(biz, 'Business Name') or 'Unknown',
                        "service": get_row_value(biz, 'Business Services') or '',
                        "location": get_row_value(biz, 'Business Location') or '',
        ...
        # Step 2: Businesses found via relational tables
        try:
            svc_hits = await asyncio.to_thread(
                lambda: supabase.table('business_services')
                    .select('business_id')
                    .ilike('service_category', service)
                    .execute()
            )
            loc_hits = await asyncio.to_thread(
                lambda: supabase.table('business_locations')
                    .select('business_id')
                    .ilike('location', location)
                    .execute()
            )
            svc_ids = {r['business_id'] for r in (svc_hits.data or [])}
            loc_ids = {r['business_id'] for r in (loc_hits.data or [])}
            extra_ids = (svc_ids & loc_ids) - matched_ids  # in both service and location tables, not already added

            # Also: service in main table + extra location table OR extra service table + location in main table
            svc_in_main_ids = set()
            loc_in_main_ids = set()
            for biz in [b for b in businesses if get_row_value(b, 'id') not in matched_ids]:
                bid = get_row_value(biz, 'id')
                if (get_row_value(biz, 'Business Services') or '').strip().lower() == service.strip().lower():
                    svc_in_main_ids.add(bid)
                if (get_row_value(biz, 'Business Location') or '').strip().lower() == location.strip().lower():
                    loc_in_main_ids.add(bid)

            extra_ids |= (svc_in_main_ids & loc_ids) - matched_ids
            extra_ids |= (svc_ids & loc_in_main_ids) - matched_ids

            if extra_ids:
                biz_map = {get_row_value(b, 'id'): b for b in businesses}
            ...
            logging.warning(f"Could not query relational tables for catalog: {e}")

        results.sort(key=lambda x: (not x['is_boosted'], (x['business_name'] or '').lower()))
        return {"service": service, "location": location, "businesses": results, "total": len(results)}

    elif service:
        # Return unique locations for this service (main table + location table)
        locations = set()
        businesses = get_cached_businesses()
        for biz in businesses:
            biz_service = (get_row_value(biz, 'Business Services') or '').strip().lower()
            biz_location = (get_row_value(biz, 'Business Location') or '').strip()
            if biz_service == service.strip().lower() and biz_location:
                locations.add(biz_location)

        # Also: businesses that offer this service via business_services table
        try:
            svc_hits = await asyncio.to_thread(
                lambda: supabase.table('business_services')
                    .select('business_id')
            ...
            extra_biz_ids = {r['business_id'] for r in (svc_hits.data or [])}
            if extra_biz_ids:
                # Get their locations from main table and from business_locations
                biz_map = {get_row_value(b, 'id'): b for b in businesses}
                for bid in extra_biz_ids:
                    biz = biz_map.get(bid)
                    if biz:
                        loc = (get_row_value(biz, 'Business Location') or '').strip()
                        if loc:
                            locations.add(loc)
                loc_hits = await asyncio.to_thread(
                    lambda: supabase.table('business_locations')
                        .select('location')
                        .in_('business_id', list(extra_biz_ids))
                        .execute()
                )
                for r in (loc_hits.data or []):
                    if r.get('location'):
                        locations.add(r['location'].strip())
        except Exception as e:
            logging.warning(f"Could not query relational tables for locations: {e}")

        return {"service": service, "locations": sorted(list(locations))}

    else:
        # Return unique services from BOTH tables
        services = {}

        # From main businesses table
        businesses = get_cached_businesses()
        for biz in businesses:
            svc = (get_row_value(biz, 'Business Services') or '').strip()
            if svc:
                key = svc.lower()
                services[key] = services.get(key, {"name": svc, "count": 0})
                services[key]["count"] += 1

        # From business_services table
        try:
            extra_svc = await asyncio.to_thread(
                lambda: supabase.table('business_services').select('service_category').execute()
            )
            for r in (extra_svc.data or []):
                svc = (r.get('service_category') or '').strip()
                if svc:
                    key = svc.lower()
                    services[key] = services.get(key, {"name": svc, "count": 0})
                    services[key]["count"] += 1
        except Exception as e:
            logging.warning(f"Could not query business_services table: {e}")
...
async def get_pay_config():
    """Return Paystack public key from server environment.
    This avoids exposing the key in URLs, browser history, and referrer headers."""
    pkey = os.getenv("PAYSTACK_PUBLIC_KEY", "")
    if not pkey:
        raise HTTPException(status_code=500, detail="Payment configuration missing")
    return {"public_key": pkey}
...
def _compress_image_for_telegram(
    data: bytes,
    max_dim: int = 1600,
    quality: int = 82
) -> bytes:
    """
    Downscale + recompress images to reduce Telegram processing timeouts.
    - max_dim: max width/height
    - quality: JPEG quality
    Returns original bytes if processing fails.
    """
    ...
    if photos and bot_instance:
        for photo in photos:
            try:
                ...
                logging.error(f"Error processing photo '{getattr(photo, 'filename', 'unknown')}': {e}")
...
@app.post("/api/business/review")
async def submit_review(request: Request):
    """Submit a review for a business. Reviews require admin approval."""
    from bot_state import get_cached_businesses, get_row_value
    ...
    if not business_id or not reviewer_id or not stars:
    ...
    # Self-review prevention: check if reviewer owns this business OR any business under same ID
    cached = get_cached_businesses()
    ...
    business_owner_id = str(get_row_value(target_business, 'Telegram ID') or '')
    reviewer_str = str(reviewer_id)
    
    if business_owner_id and business_owner_id == reviewer_str:
        raise HTTPException(status_code=403, detail="You cannot review your own business")
    
    biz_name = get_row_value(target_business, 'Business Name') or 'Unknown'
    
    try:
        # Store review with pending status
        review_data = {
            "business_id": int(business_id),
            "reviewer_telegram_id": int(reviewer_id),
            "stars": stars,
            "review_text": review_text,
            "status": "pending"
        }
        result = await asyncio.to_thread(
            lambda: supabase.table('business_reviews').insert(review_data).execute()
        )
        
        review_id = result.data[0]['id'] if result.data else 0
        ...
        if bot_instance and admin_id:
            ...
            msg = (
                ...
                f"📊 *Rating:* {star_display} ({stars}/5)\n\n"
                f"📝 *Review:*\n{review_text}\n\n"
                f"👤 *Reviewer:* `{reviewer_id}`\n"
            )
            
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Confirm Review", callback_data=f"rev_approve_{review_id}"),
                 InlineKeyboardButton("❌ Reject Review", callback_data=f"rev_reject_{review_id}")]
            ])
            ...
            try:
                ...
                logging.error(f"Failed to notify admin of review: {e}")
...
async def get_claim_info(token: str):
    """Get business info for a claim token (used by claim.html)."""
    from bot_state import get_cached_businesses, get_row_value
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to load claim info")
