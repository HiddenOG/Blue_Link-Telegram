...
app = FastAPI(title="BlueBot Dashboard API", docs_url=None, redoc_url=None)
...
_RATE_LIMITS = {"POST": 10, "GET": 60}  # requests per minute
...
def extract_user_from_initdata(init_data: str):
    """Secondary auth: extract user from initData even if HMAC fails.
    Still secure — attacker would need valid Telegram initData format.
    ...
    try:
        ...
        if "user" in parsed:
            ...
            if uid:
                logging.warning(f"⚠️ Using initData user extraction fallback for user {uid}")
                return uid
    except:
        pass
    return None
...
def resolve_user_id(init_data: str = None, user_id: int = None) -> int:
    """Resolve user identity. HMAC validation is required in production."""
    skip_auth = os.getenv('DASHBOARD_SKIP_AUTH', 'false').lower() == 'true'
    
    # 1. Try HMAC validation (always preferred)
    if init_data:
        try:
            user = validate_telegram_data(init_data)
            return user["user"]["id"]
        except:
            # Fallback: extract user from initData format
            uid = extract_user_from_initdata(init_data)
            if uid:
                return uid
    
    # 2. Only trust uid param in dev mode
    if skip_auth and user_id:
        logging.warning(f"⚠️ Using uid fallback (DEV MODE ONLY): {user_id}")
        return user_id
...
async def get_dashboard_data(initData: str = Query(None), user_id: int = Query(None)):
    ...
    if initData:
        try:
            ...
            # Fallback: extract user from initData
            uid = extract_user_from_initdata(initData)
            if uid:
                valid_user_id = uid
                try:
    ...
    # Only allow uid fallback in dev mode
    if not valid_user_id and user_id and skip_auth:
        ...
        try:
            ...
            user_name = "User"
        logging.warning(f"⚠️ Dashboard: Using uid fallback (DEV MODE): {valid_user_id}")
...
async def get_business_profile(business_id: int, viewer_id: int = Query(None)):
    ...
    # Determine if viewer is the business owner (for self-review prevention)
    is_own_business = False
    if viewer_id and owner_id:
        try:
            is_own_business = str(viewer_id) == str(owner_id)
        except:
            pass
...
async def contact_business(request: Request):
    ...
    # Auth: validate initData to get real customer_id
    ...
    if init_data:
        try:
            ...
            # Fallback: extract user from initData format
            customer_id = extract_user_from_initdata(init_data)
    
    if not customer_id and skip_auth:
...
async def report_business(request: Request):
    ...
    # Auth: validate initData to get real reporter_id
    init_data = body.get('initData')
    skip_auth = os.getenv('DASHBOARD_SKIP_AUTH', 'false').lower() == 'true'
    reporter_id = None
    ...
    if init_data:
        try:
            ...
            # Fallback: extract user from initData format
            reporter_id = extract_user_from_initdata(init_data)
    
    if not reporter_id and skip_auth:
...
async def submit_review(request: Request):
    ...
    # Auth: validate initData to get real reviewer_id
    init_data = body.get('initData')
    skip_auth = os.getenv('DASHBOARD_SKIP_AUTH', 'false').lower() == 'true'
    reviewer_id = None
    ...
    if init_data:
        try:
            ...
            # Fallback: extract user from initData format
            reviewer_id = extract_user_from_initdata(init_data)
    
    if not reviewer_id and skip_auth:
